...
- [ ] Site updated
  ...
  - [ ] Run `make feature-shortcode-update version={version}` to resolve feature shortcodes naming a version <= {version}: remove `feature expiryVersion` tags and content in both .md and .html files, and unwrap `feature publishVersion` tags while keeping their content in .md files. Keep the `publishVersion` wrapper in `agones_crd_api_reference.html`, which `gen-api-docs.sh` uses to locate the previous API docs.
