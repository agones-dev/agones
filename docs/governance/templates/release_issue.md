...
- [ ] Site updated
  ...
  - [ ] Run `make feature-shortcode-update version={version}` to remove all `feature expiryVersion` shortcodes (tags and content) and unwrap all `feature publishVersion` shortcodes (tags only, content kept) whose version is <= {version}, across both .md and .html files in site/content/en/docs.
